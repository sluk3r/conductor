...
public class ConductorServer {
	...
	private static void createKitchenSink(int port) throws Exception {
		...
		for(int i = 0; i < 40; i++) {//wxc pro 2018-7-14:18:29:17 这里的40个定义考虑哪些因素？
			taskDefs.add(new TaskDef("task_" + i, "task_" + i, 1, 0));
		}
		taskDefs.add(new TaskDef("search_elasticsearch", "search_elasticsearch", 1, 0)); //wxc 2018-7-14:18:30:30 从名字search_elasticsearch，理解Task类的定义。
		...
		client.resource("http://localhost:" + port + "/api/metadata/workflow").type(MediaType.APPLICATION_JSON).post(stream);//wxc pro 2018-7-14:18:31:14 这里直接使用REST方式， 而不是使用API方式。看背后的Controller监听到这个HTTP请求后， 怎么转换成自身的API。
		
		stream = Main.class.getResourceAsStream("/sub_flow_1.json"); //wxc 2018-7-14:18:32:44 除kitchensink外， 还定义了sub_flow_1， 这个sub_flow_1跟kitchensink的关联关系？
